        ...
        for label, raw, expected in cases:
            ...
            if isinstance(expected, tuple) and expected[0] == SUPPR:
                ...
                if actual != expected or suppression is not None:
                    failures.append(
                        f"{field}/{label}: want {expected!r}, got value={actual!r} suppression={suppression!r}"
                    )
