...
def test_override_does_not_mutate_base_glossary():
    ...
    assert _get_field_feature_available(dev_patched, "screen_display") == "excluded"
    # Second device uses the same on-disk glossary — unpatched.
    assert _get_field_feature_available(dev_clean, "screen_display") != "excluded"
...
def test_meta_override_silently_stripped():
    ...
    assert _get_field_feature_available(device, "screen_display") == "excluded"
