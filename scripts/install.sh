...
echo ""
echo "    • Bluetooth on Pi Zero W / Zero 2 W / 3 / 4 (holds the PL011 UART; the"
echo "      header pins get the weaker mini UART) — move it off with:"
echo "        Add 'dtoverlay=disable-bt' to /boot/firmware/config.txt"
echo "        sudo systemctl disable hciuart"
echo "      (Pi Zero, 1, 2: nothing to do.)"
echo ""
echo "    • Pi 5: /dev/serial0 is the 3-pin debug header, not header pins 8/10."
echo "        Add 'dtoverlay=uart0-pi5' to /boot/firmware/config.txt and use"
echo "        /dev/ttyAMA0 as the serial port (untested)."
echo ""
echo "    • Other UART services (GPS daemons, Zigbee bridges, etc.)"
echo "        Check: sudo lsof /dev/serial0 /dev/ttyAMA0 2>/dev/null"
echo ""
echo "  The AC's dongle port is 5 V; the Pi's UART pins are 3.3 V. Wire them through a"
echo "  bidirectional level shifter and power the Pi from its own supply — see"
echo "  docs/operations.md section 1 in the repository."
echo ""
